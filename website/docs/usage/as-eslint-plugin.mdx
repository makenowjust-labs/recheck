...
In addition, `ignoreErrors`, `premittableComplexities` and `cache` are only available in ESLint plugin.
...
- `cache.strategy`:
  ...
  - `'aggressive'` (default): Cache all results. Some checker (e.g. fuzzing checker) cause false-negative, so it is a bit unsafe.
  - `'conservative'`: Cache only correct results. In other words, only results reported by the automaton checker are cached.
