...
class PatternExtensionsSuite extends munit.FunSuite {
  ...
  test("PatternExtensions.NodeOps#isEmpty") {
    ...
    assertEquals(Repeat(Quantifier.Exact(0, false), Dot()).isEmpty, true)
    assertEquals(Repeat(Quantifier.Exact(0, false), Sequence(Seq.empty)).isEmpty, true)
    assertEquals(Repeat(Quantifier.Exact(1, false), Dot()).isEmpty, false)
    assertEquals(Repeat(Quantifier.Exact(1, false), Sequence(Seq.empty)).isEmpty, true)
    assertEquals(Repeat(Quantifier.Unbounded(0, false), Dot()).isEmpty, true)
    assertEquals(Repeat(Quantifier.Unbounded(0, false), Sequence(Seq.empty)).isEmpty, true)
    assertEquals(Repeat(Quantifier.Unbounded(1, false), Dot()).isEmpty, false)
    assertEquals(Repeat(Quantifier.Unbounded(1, false), Sequence(Seq.empty)).isEmpty, true)
