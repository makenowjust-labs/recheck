...
object RecallValidator {
  ...
  def validate(source: String, flags: String, pattern: AttackPattern, timeout: Duration)(
      exec: (String, Option[FiniteDuration]) => Option[(Int, String, String)]
  )(implicit ctx: Context): RecallResult = ctx.interrupt {
  ...
  private[recall] def result(output: Option[(Int, String, String)])(implicit ctx: Context): RecallResult =
    ctx.interrupt {
      output match {
        case Some((0, out, _)) =>
          val time = out.toLongOption.getOrElse(throw new UnexpectedException("invalid recall validation output"))
