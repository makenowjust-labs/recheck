...
import codes.quine.labs.recheck.unicode.CaseMap.*
...
final case class IChar(set: IntervalSet[UChar]) extends Ordered[IChar]:
  ...
  def complement(unicode: Boolean): IChar =
    val (xys, z) = set.intervals.foldLeft((IndexedSeq.empty[(UChar, UChar)], UChar(0))):
      case ((seq, x), (y, z)) => (seq :+ (x, y), z)
    IChar(IntervalSet.from(xys :+ (z, UChar(if unicode then 0x110000 else 0x10000))))

  /** Computes a union of two interval sets. */
  infix def union(that: IChar): IChar = IChar(set.union(that.set))

  /** Computes a difference interval set. */
  infix def diff(that: IChar): IChar = partition(that).diffThat
  ...
  override def toString: String =
    ...
    def showUCharInClass(u: UChar): String =
      ...
      else if 1 <= u.value && u.value < 32 then s"\\c${(u.value + 0x40).toChar}"
      else u.toString
    val cls = set.intervals.map:
      case (x, y) if x.value + 1 == y.value =>
        showUCharInClass(x)
      case (x, y) =>
        s"${showUCharInClass(x)}-${showUCharInClass(UChar(y.value - 1))}"
    cls.mkString("[", "", "]")
...
object IChar:
  ...
  def dot(ignoreCase: Boolean, dotAll: Boolean, unicode: Boolean): IChar =
    ...
    val dot = if dotAll then any else any.diff(IChar.LineTerminator)
    if ignoreCase then IChar.canonicalize(dot, unicode) else dot

  /** Creates an interval set ranged in [begin, end]. */
  def range(begin: UChar, end: UChar): IChar =
    IChar(IntervalSet((begin, UChar(end.value + 1))))
  ...
  private val canonicalizeCache: LRUCache[(IChar, Boolean), IChar] =
    new LRUCache(1000)
  ...
  def canonicalize(c: IChar, unicode: Boolean): IChar =
    canonicalizeCache.getOrElseUpdate((c, unicode)):
      val conversions = if unicode then CaseMap.Fold else CaseMap.Upper
      val set = conversions.foldLeft(c.set):
        case (set, Conversion(dom, offset)) =>
