...
import codes.quine.labo.recheck.unicode.LRUCache.LRUMap

/** LRUCache is a LRU (last recent used) cache. */
private[unicode] class LRUCache[K, V](val maxSize: Int) {
  private[this] val map: LRUMap[K, V] = new LRUMap(maxSize)
  ...
  def get(k: K): Option[V] =
    synchronized(if (map.containsKey(k)) Some(map.get(k)) else None)
