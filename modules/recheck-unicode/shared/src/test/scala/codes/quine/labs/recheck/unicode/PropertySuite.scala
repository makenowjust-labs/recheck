...
class PropertySuite extends munit.FunSuite:
  test("Property.binary"):
    ...
    for name <- Property.BinaryPropertyNames do assert(Property.binary(name).isDefined)
    for name <- Property.BinaryPropertyAliases.keys do assert(Property.binary(name).isDefined)
  ...
  test("Property.generalCategory"):
    ...
    for value <- Property.GeneralCategoryValues do assert(Property.generalCategory(value).isDefined)
    for value <- Property.GeneralCategoryValueGroups.keys do assert(Property.generalCategory(value).isDefined)
    for value <- Property.GeneralCategoryValueAliases.keys do assert(Property.generalCategory(value).isDefined)
  ...
  test("Property.script"):
    ...
    for value <- Property.ScriptValues do assert(Property.script(value).isDefined)
    for value <- Property.ScriptValueAliases.keys do assert(Property.script(value).isDefined)
  ...
  test("Property.scriptExtensions"):
    ...
    for value <- Property.ScriptValues do assert(Property.scriptExtensions(value).isDefined)
    for value <- Property.ScriptValueAliases.keys do assert(Property.scriptExtensions(value).isDefined)
