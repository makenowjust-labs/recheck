...
class IRCompilerSuite extends munit.FunSuite {
  ...
  locally {
    ...
    test("IRCompiler.compile: CharacterClass") {
      ...
      val pattern4 =
        Pattern(Sequence(Seq(LineBegin, CharacterClass(false, Seq(Character('A'))))), flagSet4) // ignoreCase + unicode
      assertEquals(
        IRCompiler.compile(pattern1),
        Success(
          IR(
    ...
    test("IRCompiler.compile: BackReference") {
      ...
      val pattern2 = Pattern(Sequence(Seq(LineBegin, Capture(1, Dot), LookBehind(false, BackReference(1)))), flagSet1)
